...
def apply_ros_rus(pos_data, neg_data, ros_rate, rus_rate):
  ...
  pos = pos_data.sample(frac=ros_rate, replace=True) if ros_rate != None else pos_data
  neg = neg_data.sample(frac=rus_rate, replace=replaceNeg) if rus_rate != None else neg_data
...
perf_metrics = ['key', 'depth', 'width',  'tp', 'fp', 'tn', 'fn', 'tpr', 'tnr', 'roc_auc', 'geometric_mean', 'arithmetic_mean', 'f1_score', 'precision']
...
def write_dnn_perf_metrics(y_true, y_prob, threshold, key, depth, width, path):
  ...
  results = [key, depth, width, tp, fp, tn, fn, tpr, tnr, roc_auc, geometric_mean, arithmetic_mean, f1, precision]
...
def write_perf_metrics(path, y_true, y_prob, time, max_depth, embedding_type):
  cols = [
    'method', 'time',
    'tp', 'fp', 'tn', 'fn','tpr', 'tnr', 'roc_auc', 'geometric_mean',
    'arithmetic_mean', 'f1_score', 'precision']
  ...
  results = [embedding_type, time,
    tp, fp, tn, fn, tpr, tnr, roc_auc, geometric_mean, arithmetic_mean, f1, precision]
  results = [rounded_str(x) for x in results]
  out += '\n' + ','.join(results)
